 ...
 * @property {string} filename - The name of the collection.
 * @property {string} basename - The name of the collection.
 * @property {number} lastmod - The creation date of the collection.
 * @property {string} size - The number of items in the collection.
 */
...
const mutations = {
	 ...
	 * @param {object} state vuex state
	 * @param {object} data destructuring object
	 * @param {Collection} data.collection the collection to update
	 */
...
const getters = {
	...
	collectionsWithPrefix: state => prefix => Object.values(state.collections)
		.filter(collections => collections.filename.search(RegExp(`^${prefix}`)) !== -1)
...
const actions = {
	 ...
	 * @param {object} data destructuring object
	 * @param {Collection[]} data.collections list of collections
	 */
	 ...
	 * @param {object} context vuex context
	 * @param {object} data destructuring object
	 * @param {Collection} data.collection the collection
	 */
	async createCollection(context, { collection }) {
		try {
			...
			logger.error(t('photos', 'Failed to create {collectionName}.', { collectionName: collection.filename }), { error })
			showError(t('photos', 'Failed to create {collectionName}.', { collectionName: collection.filename }))
		}
	},
